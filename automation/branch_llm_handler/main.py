...
def get_file_content(file_path, ref):
    ...
    print(f"Attempting to retrieve content of file '{file_path}' from ref '{ref}'")
    try:
        ...
        print(f"Successfully retrieved content from '{ref}'")
        return content
...
def process_file(file_path, commit_hash, temp_folder, index):
    try:
        ...
        if original_content is not None or new_content is not None:
            ...
            print(f"Skipping file '{file_path}' as it doesn't exist in either the commit or the current branch.")
...
def generate_commit_message(commit_hash, csv_file_path=None):
    ...
    """
    print(f"Starting commit message generation comparing current branch with commit: {commit_hash}")
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python script_name.py <commit_hash>")
        sys.exit(1)
